...
for row in cursor:
    cityIndex = int(row[0])
cursor.execute("SELECT value FROM GCIS_Company WHERE key='mainSelectIndex'")
for row in cursor:
                ...
                for i in range(pageIndex, pageCount+1, 1):
                    ...
                    for f in factoryInfos:
                        if(currentInfoIndex >= infoPageIndex-1):
                            ...
                            ws.cell(column=2, row=newRow).value = companyTaxNo
                            ws.cell(column=3, row=newRow).value = companyStatus
                            ws.cell(column=4, row=newRow).value = companyCapital
                            ws.cell(column=5, row=newRow).value = companyPerson
                            ws.cell(column=6, row=newRow).value = companyAddress
